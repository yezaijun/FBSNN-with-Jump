...
class oneDimPIDE(FBSNNJ):
    ...
    def x_init_generator(self,sample_size):
        return np.ones((sample_size,self.dim))


    def Forward_SDE(self,t,x,dt,dw,jump):
        x1 = x + self.epsilon*x*dt+ self.theta*dw+ jump
        return x1
    
    def Backward_SDE(self,t,x,dt,dw,y,dy_dx,jump):
        y1 = y +self.epsilon*x*dt + self.theta*dy_dx*dw + jump
        return y1
...
if __name__=="__main__":
    ...
    mySolver = oneDimPIDE(T, sample_size, num_time_interval, dim,
                            model)
    ...
    # # 训练模型
    # mySolver.train(num_iterations=10000, optimizer=optimizer,checkpoint=checkpoint)
